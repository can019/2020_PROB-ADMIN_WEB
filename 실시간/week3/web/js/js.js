...
/************************global variables************************/
var specializer = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi;//특수문자
//specializer.test(test할 문자) => 반환형 true, false. 만약 test에 걸린다(특수문자 존재) -> true
var modal = document.getElementById('myModal');
var span = document.getElementsByClassName("close")[0];
var bt_confirm = document.getElementById("confirm");                                        
...
window.onclick = function(event) {
    ...
    if(event.target == bt_confirm){
        ...
        //modal 수정 -> 
        //txt를 table.php에 보냄 ->table.php에서 txt를 분석 -> StudentId.txt를 수정. 
        // StudentId.txt를 읽어서 echo '<tr></>'


        //Dom node 접근 js 내장함수(대표적).
        //parentNode;
        //nextSibling
        //firstChind
        //fistElementChild;
        ...
        if(!checkhtmlSpeicalChars(txt)){
            changeCell.innerHTML = txt;
            loadTable(makeData(changeCell));
        }
        else{
            alert('특수문자를 제거해주세요');
        }
    ...
    function checkhtmlSpeicalChars(str){
        return specializer.test(str);
...
function makeData(element){
        let changeTr = element.parentNode;

        let row = changeTr.rowIndex-1;
        let name = changeTr.firstElementChild.nextSibling;
        let StudentId = name.nextSibling;
        let age = StudentId.nextSibling;
        let grade = age.nextSibling;
        let extra = grade.nextSibling;

        let data = row+'|'+name.innerHTML+'|'+StudentId.innerHTML+'|'+age.innerHTML+'|'+grade.innerHTML+'|'+extra.innerHTML;
         return data;
}
